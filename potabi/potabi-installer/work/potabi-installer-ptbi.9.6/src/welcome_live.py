...
Messages = """To run Potabi without installing, select "Try Potabi."

To install Potabi on your computer hard disk drive, click "Install Potabi."

Note: Language selection only works when selecting "Try Potabi."
      When installing Potabi, the installation program is only in English."""
...
class Welcome:
    ...
    def __init__(self, next_install_page, next_setup_install):
        ...
        self.vbox1.show()
        main_grid = Gtk.Grid()
        Title = Gtk.Label('Welcome To Potabi Systems', name="Header")
        ...
        image2.show()
        install_button = Gtk.Button(label='Install Potabi', image=image1,
                                    image_position=2)
        install_button.set_always_show_image(True)
        install_button.connect("clicked", self.install_system)
        try_button = Gtk.Button(label='Try Potabi', image=image2,
                                image_position=2)
